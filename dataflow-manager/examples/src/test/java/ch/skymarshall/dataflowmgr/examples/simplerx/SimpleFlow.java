...
public class SimpleFlow extends ch.skymarshall.dataflowmgr.examples.simple.AbstractFlow {
	...
	public class FlowExecution {
		...
		private void setSimpleExternalAdapterGetCompletioncompleteMustComplete(
				java.lang.String simpleExternalAdapter_getCompletioncomplete_mustComplete) {
	...
	// ------------------------- inputDataPoint -> simpleService.init ->
	// simpleService_init -------------------------
	private Maybe<FlowExecution> binding_simpleService_init(FlowExecution execution,
			final Function<Maybe<FlowExecution>, Maybe<FlowExecution>> callModifier, Runnable... callbacks) {
	...
	// ------------------------- simpleService_init -> simpleService.complete ->
	// complete -------------------------
	private Maybe<FlowExecution> binding_complete_mustComplete(FlowExecution execution,
			final Function<Maybe<FlowExecution>, Maybe<FlowExecution>> callModifier, Runnable... callbacks) {
		...
		Maybe<FlowExecution> callService = Maybe.just(execution)
				...
				.doOnSuccess(f -> f.setComplete(this.simpleService.complete(f.simpleService_init,
						f.simpleExternalAdapter_getCompletioncomplete_mustComplete)))
				.doOnTerminate(() -> Arrays.stream(callbacks).forEach(Runnable::run))
				.subscribeOn(Schedulers.computation());
		if (callModifier != null) {
			callService = callModifier.apply(callService);
		...
		final Maybe<FlowExecution> callAdaptersAndServiceConst = Maybe.just(execution)
				.zipWith(adapter_getCompletion, (r, s) -> execution).flatMap(r -> callServiceConst);
		...
		final Maybe<FlowExecution> activationCheck = Maybe.just(true)
				...
				.flatMap(e -> callAdaptersAndServiceConst).doOnComplete(() -> {
					execution.setStateBindingCompleteMustComplete(DataPointState.TRIGGERED);
					execution.setStateComplete(DataPointState.SKIPPED);
				}).doOnTerminate(() -> Arrays.stream(callbacks).forEach(Runnable::run));

		Maybe<FlowExecution> first = Maybe.just(execution)
				.mapOptional(f -> ((DataPointState.TRIGGERED == f.state_simpleService_init)) ? Optional.of(execution)
						: Optional.empty())
	...
	// ------------------------- simpleService_init -> simpleService.keepAsIs ->
	// complete -------------------------
	private Maybe<FlowExecution> binding_complete(FlowExecution execution,
			final Function<Maybe<FlowExecution>, Maybe<FlowExecution>> callModifier, Runnable... callbacks) {
		Maybe<FlowExecution> callService = Maybe.just(execution)
		...
		Maybe<FlowExecution> first = Maybe.just(execution)
				.mapOptional(f -> ((DataPointState.TRIGGERED == f.state_simpleService_init)
						&& (DataPointState.TRIGGERED == f.state_complete || DataPointState.SKIPPED == f.state_complete))
	...
	// ------------------------- complete -> exit -> exit -------------------------
	private Maybe<FlowExecution> binding_exit(FlowExecution execution,
			final Function<Maybe<FlowExecution>, Maybe<FlowExecution>> callModifier, Runnable... callbacks) {
		final Maybe<?> adapter_display = Maybe.just(execution)
				.doOnSuccess(f -> this.simpleExternalAdapter.display(f.complete)).subscribeOn(Schedulers.io());
		...
		final Maybe<FlowExecution> callServiceConst = callService;
		final Maybe<FlowExecution> callAdaptersAndServiceConst = Maybe.just(execution)
				.zipWith(adapter_display, (r, s) -> execution).flatMap(r -> callServiceConst);
		Maybe<FlowExecution> first = Maybe.just(execution).mapOptional(
				f -> ((DataPointState.TRIGGERED == f.state_complete)) ? Optional.of(execution) : Optional.empty())
	...
	public Maybe<FlowExecution> execute(java.lang.String inputDataPoint,
		...
		// ------------------------- simpleService_init -> simpleService.complete ->
		// complete -------------------------
		final Maybe<FlowExecution> binding_complete_mustComplete = binding_complete_mustComplete(execution, null,
				() -> binding_complete.subscribe(), () -> binding_exit.subscribe());
		// ------------------------- inputDataPoint -> simpleService.init ->
		// simpleService_init -------------------------
		final Maybe<FlowExecution> binding_simpleService_init = binding_simpleService_init(execution, null,
				() -> binding_complete_mustComplete.subscribe(), () -> binding_complete.subscribe());
