...
public:
	...
	virtual void handle_exception(conversion_exception* e) = 0;
...
private:
	...
	public:
		property_link(binding_chain& _chain,
				property_setter_func_type<typed_property<_Pt>, _Pt> setter) :
				m_chain(_chain), m_setter(setter) {
		}

		void to_property(int index, const void* component, const _Pt value)
				throw (conversion_exception) {
			m_setter.set(component, value);
		}

		void to_component(int index, const _Pt value)
				throw (conversion_exception) {
			((binding_forward<_Pt>*) m_chain.m_links[index + 1]->m_binding_forward)->to_component(
					index + 1, value);
		}

		void handle_exception(conversion_exception* e) {
			// no conversion
		}
	...
	public:
		...
		void to_property(int index, const void* component, const _Tt value)
				throw (conversion_exception) {
			((binding_backward<_Ft>*) m_chain.m_links[index - 1]->m_binding_backward)->to_property(
					index - 1, component,
					m_converter->convert_component_value_to_property_value(
							value));
		}

		void to_component(int index, const _Ft value)
				throw (conversion_exception) {

			((binding_forward<_Tt>*) m_chain.m_links[index + 1]->m_binding_forward)->to_component(
					index + 1,
	...
	public:
		...
		void set_value_from_component(void* component, _Ct componentValue) {
			...
			int lastIndex = m_chain.m_links.size() - 1;
	...
	public:
		chain_component_link(binding_chain& _chain,
			...
			m_componentBinding->add_component_value_change_listener(
					new component_link_impl<_Ct>(_chain, m_componentBinding));
		}
...
public:
	...
	end_of_chain<_Pt>* bindProperty(
		...
		property_link* const link = new property_link(*this, setter);
		m_links.push_back(new binding_storage((void*)static_cast<binding_forward<_Pt>*>(link), (void*)static_cast<binding_backward<_Pt>*>(link)));
	...
	template<class _FT, class _TT> end_of_chain<_TT>* bind(
		...
		converter_link<_FT, _TT>* const link = new converter_link<_FT, _TT>(
				*this, _converter);
		m_links.push_back(new binding_storage((void*)static_cast<binding_forward<_FT>*>(link), (void*)static_cast<binding_backward<_TT>*>(link)));
	...
	template<class _Ct> void* bind(component_binding<_Ct>* _componentBinding) {
		...
		m_links.push_back(new binding_storage((void*)static_cast<binding_forward<_Ct>*>(link), (void*)static_cast<binding_backward<_Ct>*>(link)));
		return NULL;
	}
