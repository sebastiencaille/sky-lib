...
public class ListModelContent<T extends @Nullable Object> extends AbstractListModel<T>
    ...
    private class Edition implements IEdition<T> {
        ...
        /**
         * True if the filter does not filter out the object
         */
        @Getter
        boolean accepted;
...
//	just a marker
    ...
    private class ContentListeners implements IListViewOwner<T>, PropertyChangeListener, IChildModelListener<T>, Serializable {
        ...
        public void editionStarted(final ListEvent<T> event) {
            final var oldIndex = getRowOf(event.getObject());
            startEdition(event.getObject(), oldIndex);
        }
     ...
     * The current view
     */
    private final ObjectProperty<IListView<T>> viewProperty = new ObjectProperty<>("View", changeSupport, ListViews.simple());
    ...
    @Setter
    private @Nullable ListModel<T> base;
    ...
    public Optional<T> remove(final int row) {
        ...
        return removeFromModel(value);
    }

    @Override
    public Optional<T> getEditedValue() {
        return Optional.ofNullable(objectEdition).map(o -> o.value);
    }
    ...
    public JavaExt.CloseableOptional<IEdition<T>> startEditingValue(final T value) {
        ...
        if (oldIndex < 0) {
            return JavaExt.ofNullable(null);
        }
        return JavaExt.ofNullable(startEdition(value, oldIndex));
    }

    private Edition startEdition(T value, int oldIndex) {
        checkNoEdition();
    ...
    public JavaExt.CloseableOptional<IEdition<T>> findForEdition(final T sample) {
        ...
        return JavaExt.ofNullable(startEdition(data.get(row), row));
    }
    ...
    public IEdition<T> findOrCreateForEdition(final T sample) {
        ...
        return startEdition(edited, row);
    }
