...
public class ExamplePage extends PagePilot {
	...
	private static final ScenarioFactory<SeleniumPilot, ExamplePage> SCENARIO_FACTORY = Scenario.of(ExamplePage::new);

	public static final Scenario<SeleniumPilot, ExamplePage> SCENARIO_CONNECT = SCENARIO_FACTORY.with(
			"I connect to the website", p -> p.getDriver().get("http://localhost:8080/example1.html"),
			"I check that the website is loaded", ExamplePage::checkEnable);

	public static final Scenario<SeleniumPilot, ExamplePage> SCENARIO_ENABLE = SCENARIO_FACTORY.with(
			"I start the Enable button test", ExamplePage::testEnable, "I check that the Enable button test worked",
			ExamplePage::checkEnable);

	public static final Scenario<SeleniumPilot, ExamplePage> SCENARIO_ALERT = SCENARIO_FACTORY.with(
			"I expect the browser Alert", ExamplePage::expectTestAlertDialog, "I trigger the Alert",
			ExamplePage::testAlert, "I check that the alert is raised", ExamplePage::checkDialogHandled);
