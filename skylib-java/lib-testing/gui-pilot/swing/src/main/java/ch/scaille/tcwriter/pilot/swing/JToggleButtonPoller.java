...
public class JToggleButtonPoller extends SwingPollingBuilder<JToggleButton, JToggleButtonPoller, JToggleButtonPoller.SwingPoller> {

	public static class SwingPoller extends ch.scaille.tcwriter.pilot.swing.SwingPollingBuilder.SwingPoller<JToggleButton> {
		...
		public boolean isSelected(final boolean expected) {
			return configure(polling -> polling.withReportText(checkingThat("component is " + (expected ? "selected" : "not selected"))))
	...
	public void assertSelected(final boolean expected) {
		fail().ifNot().isSelected(expected);
	}

	public void setSelected(final boolean selected) {
		fail().ifNot().setSelected(selected);
	}
