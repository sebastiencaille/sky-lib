...
import ch.skymarshall.tcwriter.test.ITestExecutionController;
...
public class MyTC {
	...
	public void testCase() throws Exception {
		...
		// Step 1: As customer, I go in a local shop and buy a coffee machine (of brand:
		// DeLuxeBrand)
		testExecutionController.beforeStepExecution(1);
		final ch.skymarshall.tcwriter.examples.api.interfaces.selectors.BuyItemSelector var1 = BuyItemSelector
				.inLocalShop();
		final ch.skymarshall.tcwriter.examples.api.interfaces.dto.TestItem var2 = TestItem.coffeeMachine();
		var2.setBrandName("DeLuxeBrand");
		customer.buy(var1, var2);

		testExecutionController.afterStepExecution(1);

		// Step 2: As customer, I get the package bought at the shop and check that the
		// packaged item is a coffee machine of brand "DeLuxeBrand"
		testExecutionController.beforeStepExecution(2);
		final ch.skymarshall.tcwriter.examples.api.interfaces.selectors.HandlePackageSelector var5 = HandlePackageSelector
				.fromShop();
		final ch.skymarshall.tcwriter.examples.api.interfaces.dto.TestItem var6 = TestItem
				.coffeeMachineOfBrand("DeLuxeBrand");
		...
		final java.lang.String anotherBrand = customer.findAnotherBrand();

		testExecutionController.afterStepExecution(4);

		// Step 5: As customer, I keep the note "<the value anotherBrand provided by
		// step 4>"
