...
public class TestCaseRecorder implements ITestCaseRecorder {
        ...
        for (int i = 0; i < apiArgs.length; i++) {
            ...
            if (parameterValue != null) {
                ...
                step.getParametersValue().add(
                        new TestParameterValue(actionParameter, simpleParameter, Objects.toString(apiArg)));
            }
        }
    ...
    public void recordParamFactoryCall(Object factory, Method api, Object[] args) {
        ...
        final var testParameterFactory = testParameterValue.getParameterValueFactory();
        Object apiArg;
        if (args.length > 0) {
            apiArg = args[0];
        } else {
    ...
    private TestParameterValue createFactoryParameterValue(final TestApiParameter param, @Nullable final Object apiArg) {
        return new TestParameterValue(param, simpleType(param.getParameterType()), Objects.toString(apiArg));
    }
    ...
    public TestCase buildTestCase(final String testClassName) {
        ...
        testCase.getMetadata().setDescription("%s: execution at %s".formatted(testClassName,
               EXECUTION_FORMATTER.format(LocalDateTime.now())));
        ...
        testParameterValues.values().stream().map(TestParameterValue::getParameterValueFactory)
