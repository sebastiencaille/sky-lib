...
public class TestCaseToJavaVisitor {
	...
	private String visitTestParameterValue(final JavaCodeGenerator<RuntimeException> testCode,
		...
		if (hasOptionalParameters) {
			...
			return requireNonNull(parameterVarName);
		}
		return mandatoryParamContentCode.toString();
	}

	private void addParameterValuesToCall(
			@Nullable final JavaCodeGenerator<RuntimeException> javaContent,
	...
	private void inlineValue(@Nullable final JavaCodeGenerator<RuntimeException> stepContentCode,
							 final JavaCodeGenerator<RuntimeException> parametersContent,
							 final TestStep step,
							 @Nullable final TestDictionary dictionary,
		...
		case TestParameterFactory f when f.getNature() == ParameterNature.TEST_API ->
			parametersContent.append(visitTestParameterValue(requireNonNull(stepContentCode, "Required by TestApi"),
					requireNonNull( dictionary, "Required by TestApi"), step, parameterValue));
